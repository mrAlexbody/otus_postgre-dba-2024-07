...
>> Потому-что при UPDATE данные строки помечаются как удаленные в таблице, а новые помечаются как новые. 
> > Так как AUTOVACUUM отключён, нет механизма для обновления форматов записи в таблицах как свободные для переиспользования.
> > Следовательно растёт объём данных на диске при UPDATE, может помочь только полный VACUUM всей БД.
Включим AUTOVACUUM: 
```postgresql
